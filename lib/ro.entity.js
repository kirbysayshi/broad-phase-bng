(function(exports){
	...
	Entity.prototype.accelerate = function(dt){
		// apply acceleration to current position
		this.pos.x += this.accel.x * dt*dt;
		this.pos.y += this.accel.y * dt*dt;
	...
	Entity.prototype.draw = Entity.prototype.debugDraw = function(){
		var dpos = this.world.screen.dPos;
		
		this.world.screen.ctx.drawRect( 
			 dPos( this.pos.x ), dPos( this.pos.y )
			,dPos( this.size.x ), dPos( this.size.y )
		);
	}
